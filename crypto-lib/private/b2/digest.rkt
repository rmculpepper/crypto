...
(define b2s-digest-impl%
  (class digest-impl%
    (inherit get-size)
    (super-new)
    (define/override (-digest-buffer key inbuf instart inend)
      (define outbuf (make-bytes (get-size)))
      (blake2s outbuf (ptr-add inbuf instart) (- inend instart)
...
(define b2b-digest-impl%
  (class digest-impl%
    ...
    (define/override (-digest-buffer key inbuf instart inend)
      (define outbuf (make-bytes (get-size)))
      (blake2b outbuf (ptr-add inbuf instart) (- inend instart)
               key (if key (bytes-length key) 0))
      outbuf)
    (define/override (-new-ctx key)
