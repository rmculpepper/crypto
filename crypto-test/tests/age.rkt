...
(test #:name "age"
  ...
  (check (age-decrypt (list sk1) enc1) #:is msg)
  (check (age-decrypt (list sk2) enc2) #:is msg)
  (check (age-decrypt (list sk1 sk2) enc1) #:is msg)
  (check (age-decrypt (list sk1) enc12) #:is msg)
  (check (age-decrypt (list sk1) enc2)
         #:error #rx"age-decrypt: no identity matched")
  (check (age-decrypt (list `(scrypt ,pass)) enc12)
         #:error #rx"age-decrypt: no identity matched")

  (check (age-decrypt (list `(scrypt ,pass)) encp) #:is msg)

  (check (age-encrypt (list sk1 `(scrypt ,pass)) msg)
         #:error #rx"age-encrypt: scrypt recipient ")
  (check (age-encrypt (list `(scrypt ,pass) `(scrypt #"other")) msg)
         #:error #rx"age-encrypt: scrypt recipient ")

  ;; ----------------------------------------
  (test #:name "fixed key generated by age-keygen"
    ...
    (check (age-decrypt (list ska) enca) #:is #"fork in the road"))

  ;; ----------------------------------------
  (test #:name "compatibility with age and age-keygen"
    ...
    (define-values (pks3 sks3 pk3 sk3)
      (cond [age-keygen-exe
            ...
            [else
             (values (pk-key->datum pk1 'age/v1-public)
                     (pk-key->datum sk1 'age/v1-private)
                     pk1 sk1)]))
    (when age-exe
      (define enc3
      ...
      (check (age-decrypt (list sk3) enc3) #:is msg))
    (when age-exe
  ...
  ;; ----------------------------------------
  (test #:name "reject low scrypt work factors"
    (let ([low-work-age (new age% (scrypt-ln 3))])
      (define enc
        (let ([out (open-output-bytes)])
          (send low-work-age age-encrypt
                (list `(scrypt ,pass)) (open-input-bytes msg) out)
          (get-output-bytes out)))
      (check (age-decrypt (list `(scrypt ,pass)) enc)
             #:error #rx"age-decrypt: bad scrypt stanza.*work factor")))

  ;; ----------------------------------------
  (test #:name "error if no impls"
    (check (parameterize ((crypto-factories null))
             (new age% (init-who 'age-maker)))
           #:error #rx"age-maker: no [a-z]* implementation found")
    (check (parameterize ((crypto-factories null))
             (age-encrypt (list pk1) msg))
           #:error #rx"age-encrypt: no [a-z]* implementation found")))
