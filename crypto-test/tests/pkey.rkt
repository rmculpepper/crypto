  ...
  (for ([key-sexpr private-keys]
    ...
    (test #:name (format "pkey ~a" desc)
      ...
      (test #:name (format "self ~a, ~a ~a" factory-name desc fmt)
        ...
        (check pubkey public-only-key?)
        (check-true (public-key=? key pubkey))
      ...
      (for ([pub-factory (remove factory pub-factories)]
        ...
        (test #:name (format "cross (~a => ~a)" factory-name (send pub-factory get-name))
          #:pre (unless pubkey*
...
(define (test-keygen factory spec impl)
  (define (test-rt-equal privkey)
    (when (pk-format-ok? factory (send privkey get-spec) 'rkt-private)
      ...
      (check-true (public-key=? privkey privkey2)))
    (when (pk-format-ok? factory (send privkey get-spec) 'rkt-public)
      ...
      (check-true (public-key=? privkey pubkey2))))
  (test #:name (format "~a ~a keygen" (send factory get-name) spec)
    ...
    (case spec
      ...
      [(eddsa)
       ...
       (check p pk-parameters?)
       (define k2 (generate-private-key p '()))
       (check k2 private-key?)
       (test-rt-equal k)
...
(define (test-pk-serialize key pubkey factory)
  ...
  (for ([fmt '(PrivateKeyInfo OneAsymmetricKey rkt-private)]
    ...
    (test #:name (format "serialize private ~v" fmt)
      ...
      (check key2 private-key?)
      (check-true (public-key=? key2 key))
  ...
  (for ([fmt '(SubjectPublicKeyInfo rkt-public)])
    (test #:name (format "serialize public ~v" fmt)
      ...
      (check pubkey2 public-only-key?)
      (check-true (public-key=? pubkey2 key))
      (check-true (public-key=? pubkey2 pubkey))
...
(define (test-pk-sign* key pubkey pads)
  (for* ([pad pads] [di '(sha1 sha256)])
    (test #:name (format "sign w/ pad = ~v, digest = ~v" pad di)
      #:pre (unless (and (pk-can-sign? key pad) (pk-can-sign? pubkey pad)
                         (pk-can-sign? key pad di) (pk-can-sign? pubkey pad di))
              (skip-test "unsupported pad/digest"))
      ...
      (check-true (pk-verify-digest key di (digest di* msg) sig1 #:pad pad))
      (check-true (pk-verify-digest key di (digest di* msg) sig2 #:pad pad))
      (check-true (pk-verify-digest pubkey di (digest di* msg) sig1 #:pad pad))
      (check-true (pk-verify-digest pubkey di (digest di* msg) sig2 #:pad pad))
      (check-true (digest/verify key di msg sig1 #:pad pad))
      (check-true (digest/verify key di msg sig2 #:pad pad))
      (check-true (digest/verify pubkey di msg sig1 #:pad pad))
      (check-true (digest/verify pubkey di msg sig2 #:pad pad))
      (check-false (digest/verify key di badmsg sig1 #:pad pad)))))
...
(define (test-pk-encrypt key pubkey)
  ...
  (for ([pad (if rsa? '(pkcs1-v1.5 oaep) '(#f))])
    (test #:name (format "encrypt w/ pad = ~v" pad)
      #:pre (unless (and (pk-can-encrypt? key pad) (pk-can-encrypt? pubkey pad))
              (skip-test "unsupported pad"))
      (define skey (semirandom-bytes 16))
      (define wkey (pk-encrypt pubkey skey #:pad pad))
      (with-check-info (['note "pk-decrypt"])
        (check-equal? (pk-decrypt key wkey #:pad pad) skey)))))
...
(define (test-pk-key-agree key1 pubkey1)
  ...
  (check-equal? (pk-derive-secret key1 pubkey2) (pk-derive-secret key2 pubkey1)))
