...
(require racket/class
         ...
         rackunit
         crypto
         (only-in crypto/private/common/catalog curve-name->aliases)
         "util.rkt")
...
(define (readkey sexpr factory)
  ...
  (cond [(and (send factory get-pk pkspec)
              (or (not curve) (factory-has-curve? factory curve)))
         (define privkey
...
(define (factory-has-curve? factory curve)
  (define factory-curves (send factory info 'all-curves))
  (for/or ([alias (curve-name->aliases curve)])
...
(define (test-pk factory [pub-factories null] #:keygen? [keygen? #f])
  ...
  (for ([key-sexpr private-keys]
    ...
    (match-define (list* fmt pkspec desc curve _) key-sexpr)
    (define-values (key pubkey0) (readkey key-sexpr factory))
    (unless key
      (when #t
        (hprintf -1 "skipping ~s, cannot read (~a)\n" desc factory-name)))
    (when key
      (when #t
      ...
      (when (pk-key? key)
        (define pubkey-der (pk-key->datum key 'SubjectPublicKeyInfo))
        (for ([pub-factory (remove factory pub-factories)]
              #:when (and (memq (send key get-spec) (send pub-factory info 'all-pks))
...
(define (test-pk-pubkeys factory)
  ...
  (for ([key-sexpr (in-list public-keys)])
    ...
    (when (and (send factory get-pk pkspec)
      ...
      (test-case (format "~a ~a ~a" factory-name desc fmt)
        (define pubkey (datum->pk-key keydata fmt factory))
...
'(openssh-public
  eddsa "openssh Ed25519, no comment" ed25519
...
'(PrivateKeyInfo
  dsa "DSA nbits=1024" #f ;; generated by gcrypt
...
'(PrivateKeyInfo
  dh "DH nbits=512, generator=2" #f ;; generated by libcrypto
...
'(PrivateKeyInfo
  ec "EC secp192r1" secp192r1 ;; generated by libcrypto
...
'(PrivateKeyInfo
  ec "EC secp256r1" secp256r1 ;; generated by libcrypto
...
'(PrivateKeyInfo
  eddsa "Ed448, PKI" ed448  ;; generated by libcrypto
...
'(PrivateKeyInfo
  ecx "X448, PKI" x448  ;; generated by libcrypto
...
'(PrivateKeyInfo
  rsa "openssh RSA nbits=2048, comment" #f
